function check(){
	...
	var kt =/^[A-Za-z][A-z0-9]{3,20}$/;
	var check =true;
	if (!kt.test(name)) {
		document.getElementById("ten").innerHTML="Tên đăng nhập phải không dấu, không khoảng trắng và bắt đầu bằng chữ cái,có 4 đến 20 ký tự ";
		check =false;
	}
	else{
	...
	if (!kt.test(name)) {
		document.getElementById("mk").innerHTML="Mật khẩu phải không dấu, không khoảng trắng và bắt đầu bằng chữ cái,có 4 đến 20 ký tự ";
		check = false;
	}
	else{
...
function orders(){
	var xmlhttp=new XMLHttpRequest();
	 		...
	 		xmlhttp.open("GET","thongtinddhang.php");
	 		xmlhttp.send();
}